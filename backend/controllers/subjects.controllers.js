...
export async function getAllSubjects(req, res) {
  try {
    const result = await pool.query(
      `SELECT * FROM subjects WHERE is_active = true ORDER BY subject_id ASC`,
    );
    res
      .status(200)
      .json({
...
export async function updateSubject(req, res) {
  ...
  try {
    const result = await pool.query(
      `UPDATE subjects SET name = $1, class_id = $2 WHERE subject_id = $3 RETURNING *`,
      [name, class_id, id],
    );
...
export async function deleteSubject(req, res) {
  ...
  try {
    const result = await pool.query(
      `UPDATE subjects SET is_active = false WHERE subject_id = $1 RETURNING *`,
      [id],
    );
    if (result.rowCount === 0) {
      return res.status(404).json({ message: "Subject not found" });
